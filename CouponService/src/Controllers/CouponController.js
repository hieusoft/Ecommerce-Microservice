...
class CouponController {
  ...
  async getAll(req, res) {
    try {
      ...
      res
        .status(200)
        .json({ coupons, message: "Coupons retrieved successfully" });
    } catch (error) {
  ...
  async create(req, res) {
    try {
      const pool = req.app.get("dbPool");
      const newCouponId = await CouponService.create(
        pool,
        req.body,
      ...
      res
        .status(201)
        .json({ id: newCouponId, message: "Coupon created successfully" });
    } catch (error) {
  ...
  async validateCoupon(req, res) {
    try {
      const pool = req.app.get("dbPool");
      const { coupon_code, total_price, user_id } = req.body;
